      ...
      {filteredTimelineItems.length === 0 ? (
        ...
        <div className="mt-3 space-y-3">
          {filteredTimelineItems.map((item, idx) => (
            <div key={`${item.type}-${idx}-${item.title}`} className="border rounded-lg p-3 bg-gray-50">
              <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-gray-500">{item.date.toLocaleString('pt-BR')}</span>
                </div>
                <span className="text-[11px] text-gray-500">{item.status || 'Sem status'}</span>
              </div>
